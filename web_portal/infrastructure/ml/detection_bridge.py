...
from modules.ocr import ocr_processor
from modules.ocr.ocr_manager import OCRManager

# ---------------------------------------------------------------------------
# PaddleOCR lazy loader: khởi tạo 1 lần duy nhất, cache cho các lần chạy sau
# ---------------------------------------------------------------------------
_ocr_reader = None
...
def _get_ocr_reader():
    """Trả về PaddleOCR reader đã được cache (thread-safe)."""
    global _ocr_reader
    ...
    with _ocr_reader_lock:
        ...
        _ocr_reader = PaddleOCR(use_angle_cls=False, det=True, lang='en', show_log=False)
        return _ocr_reader
...
def process_video(
    ...
    # --- Khởi tạo OCR Manager nếu bật nhận diện biển số ---
    ocr_manager: Optional[OCRManager] = None
    if enable_license_plate:
        try:
            ocr_reader = _get_ocr_reader()
            ocr_manager = OCRManager(ocr_reader, interval=4, vote_threshold=3)
            ocr_manager.start_worker()
        except Exception as exc:
            print(f"[detection_bridge] Khong the khoi tao PaddleOCR: {exc}")
            ocr_manager = None
    ...
    try:
        while True:
            ...
            frame_index += 1
            # Lưu bản sao sạch (chưa vẽ) để OCR cắt ảnh biển số chính xác
            clean_frame = frame.copy() if ocr_manager is not None else None
            ...
            current_plate_ids: set = set()

            # Thu thập bounding box xe hợp lệ cho OCR lọc biển số
            valid_vehicles: List[Tuple[int, int, int, int]] = []
            if ocr_manager is not None:
                for result in results:
                    for box in result.boxes:
                        veh_label = _canonical_label(model.names[int(box.cls[0])])
                        if veh_label in PARKING_LABELS:
            ...
            for result in results:
                for box in result.boxes:
                    ...
                    # --- OCR: Xử lý biển số xe ---
                    if label in LICENSE_PLATE_LABELS and ocr_manager is not None and track_id != -1:
                        processed = ocr_manager.process_plate(
                            frame, clean_frame, track_id,
                            x1, y1, x2, y2, center_x, center_y,
                            valid_vehicles, current_time, frame_index,
                        )
                        if processed:
                            current_plate_ids.add(processed)
                        # OCR manager tự vẽ box + text biển số, bỏ qua vẽ mặc định
                        continue
            ...
            # OCR: Vẽ grace period boxes và dọn dẹp bộ nhớ
            if ocr_manager is not None:
    ...
    # Thu thập biển số đã nhận diện từ OCR Manager
    recognized_plates: List[str] = []
    if ocr_manager is not None:
        recognized_plates = sorted(set(ocr_manager.plate_confirmed.values()))
    ...
    return {
        ...
        "total_plates_recognized": len(recognized_plates),
        "latest_status": latest_status,
