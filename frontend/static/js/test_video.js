document.addEventListener("DOMContentLoaded", () => {
    ...
    function finalizeButtons() {
        ...
        if (stopButton) {
            ...
            stopButton.textContent = "🛑 Dừng Phân tích";
        }
    }
    ...
    function renderSummary(summary) {
        const detectedPlates = summary.detected_plates || {};
        const platesCount = Object.keys(detectedPlates).length;
        
        let platesTableHtml = '';
        if (platesCount > 0) {
            const platesArray = Object.entries(detectedPlates);
            platesTableHtml = `
                <article class="summary-card summary-card-wide" style="grid-column: 1 / -1;">
                    <span>Biển Số Phát Hiện</span>
                    <div style="overflow-x: auto; margin-top: 12px;">
                        <table style="width: 100%; font-size: 0.85rem;">
                            <thead>
                                <tr style="border-bottom: 1px solid var(--border);">
                                    <th style="text-align: left; padding: 8px;">Biển Số</th>
                                    <th style="text-align: center; padding: 8px;">Lần</th>
                                    <th style="text-align: center; padding: 8px;">Độ Chính Xác</th>
                                    <th style="text-align: center; padding: 8px;">Frame</th>
                                </tr>
                            </thead>
                            <tbody>
                                ${platesArray.map(([plate, data]) => `
                                    <tr style="border-bottom: 1px solid var(--border);">
                                        <td style="padding: 8px;">
                                            <code style="background: var(--primary); color: white; padding: 4px 8px; border-radius: 4px;">
                                                ${plate}
                                            </code>
                                        </td>
                                        <td style="text-align: center; padding: 8px;">${data.count || 0}</td>
                                        <td style="text-align: center; padding: 8px;">${((data.avg_confidence || 0) * 100).toFixed(1)}%</td>
                                        <td style="text-align: center; padding: 8px; color: var(--text-muted);">
                                            ${data.first_seen_frame}~${data.last_seen_frame}
                                        </td>
                                    </tr>
                                `).join('')}
                            </tbody>
    ...
    if (stopButton) {
        stopButton.addEventListener("click", async () => {
            ...
            try {
                ...
                window.portalApi.showToast("Đã gửi yêu cầu dừng phân tích", "info");
            } catch (err) {
                window.portalApi.showNotice(feedback, err.message, "error");
                stopButton.disabled = false;
                stopButton.textContent = "🛑 Dừng Phân tích";
            }
        });
