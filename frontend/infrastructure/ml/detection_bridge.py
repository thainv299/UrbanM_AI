...
from .ocr_license_plate import (
    ocr_read_license_plate,
    save_plate_image,
    is_valid_plate,
)
...
def process_video(
    ...
    congestion_frames = 0
    latest_status = "Đang bắt đầu phân tích video..."
    violation_events: List[Dict[str, Any]] = []
    detected_plates: Dict[str, Dict[str, Any]] = {}  # {plate: {count, confidence, timestamp, image_path}}
    plate_output_dir = Path(settings.get("plate_output_dir", "runtime/license_plates"))
            ...
            for result in results:
                for box in result.boxes:
                    ...
                    if label in LICENSE_PLATE_LABELS:
                        ...
                        # OCR: Đọc biển số từ ảnh crop
                        plate_img = frame[max(0, y1):min(frame.shape[0], y2), max(0, x1):min(frame.shape[1], x2)]
                        if plate_img is not None and plate_img.size > 0:
                            plate_text, plate_confidence = ocr_read_license_plate(plate_img)
                            ...
                            if plate_text and is_valid_plate(plate_text):
                                ...
                                detected_plates[plate_text]["confidence"].append(plate_confidence)
                                detected_plates[plate_text]["last_seen"] = frame_index
                                
                                # Lưu ảnh biển số
                                image_path = save_plate_image(plate_img, plate_text, plate_output_dir)
                                if image_path:
                                    detected_plates[plate_text]["image_path"] = str(image_path)
                                
                                # Hiển thị biển số trên video
                                display_label = f"{plate_text} ({plate_confidence:.2f})"
                                box_color = (0, 200, 255)  # Vàng cam nổi bật
    ...
    return {
        ...
        "no_parking_points": no_parking_points,
        # Thông tin biển số OCR
        "detected_plates_count": len(detected_plates),
        "detected_plates": {
            plate: {
                "count": data["count"],
                "avg_confidence": round(sum(data["confidence"]) / len(data["confidence"]), 3) if data["confidence"] else 0,
