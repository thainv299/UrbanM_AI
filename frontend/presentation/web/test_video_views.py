...
@test_video_router.get("/license-plates", name="test_video.license_plates_page")
async def license_plates_page(request: Request, user=Depends(login_required)):
    """Trang xem danh sách biển số"""
    if isinstance(user, RedirectResponse):
        return user
...
@test_video_router.get("/api/license-plates")
async def api_license_plates(
    user=Depends(login_required),
    limit: int = 100,
):
    ...
    from infrastructure.database.sqlite_db import get_detected_license_plates
...
@test_video_router.get("/api/license-plates/date/{detected_date}")
async def api_license_plates_by_date(
    detected_date: str,
    user=Depends(login_required),
):
    ...
    from infrastructure.database.sqlite_db import get_license_plate_by_date
